...
int main()
{
    ...
    int localSockRes = getsockname(server_fd, (struct sockaddr *) &local_address, &local_address_length);
    ...
    for (;;)
    {
        ...
        //getting peer ko aaba
        struct sockaddr_in peer_address;
        socklen_t peer_legth;

        int peerSockRes = getpeername(c_fd,(struct sockaddr *) &peer_address, &peer_legth);
        printf("Peer socket name: %s:%d\n", inet_ntoa(peer_address.sin_addr), ntohs(peer_address.sin_port));
