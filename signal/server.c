...
#include<sys/wait.h>    //TODO: last two libraries for signal
#include<signal.h>

//global 
int child_count = 0;
void childExitHandeler();

void handleClient(int client_fd);

int main(){
    struct sigaction myAction;
    myAction.sa_handler = childExitHandeler; //handeler

    //mask
    if(sigfillset(&myAction.sa_mask) < 0){
        perror("masking failed");
        exit(EXIT_FAILURE);
    }
    //restart
